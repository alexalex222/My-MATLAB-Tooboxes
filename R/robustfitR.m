function [ lm ] = robustfitR( X, y, wfun, intercept)
%multiple linear regression based on R interface
%   Input:
%   X: regressor matrix
%   y: response variable
%   intercept: flag to include intercept
%   Output:
%   lm: linear model structure

if nargin<2 || nargin>4
    error('2, 3 or 4 inputs only')
end

if (nargin ==2)
    wfun = 'bisquare';
    intercept = true;
end 

if (nargin ==3)
    intercept = true;
end 


openR;

evalR('library(MASS)',0);

putRdata('X', X);
evalR('X = as.matrix(X)',0);
evalR('X = data.frame(X)',0);

putRdata('y', y);
evalR('y = as.vector(y)',0);
if ( strcmp(wfun,'huber'))
    if (intercept)
        evalR('model <- rlm(y ~ ., data = X)',0);
    else
        evalR('model <- rlm(y ~ .-1, data = X)',0);
    end
elseif (strcmp(wfun, 'bisquare'))
    if (intercept)
        evalR('model <- rlm(y ~ ., data = X, method = "MM" )',0);
    else
        evalR('model <- rlm(y ~ .-1, data = X, method = "MM")',0);
    end
else
    error('No robust weight function found.');
end

evalR('weight <- model$w',0);
weight = getRdata('weight');

evalR('summarylm <- summary(model)',0)

evalR('df <- summarylm$df',0);
df = getRdata('df');

evalR('resStd <- summarylm$sigma',0);
resStd = getRdata('resStd');

evalR('temptable <- summarylm$coefficients',0);
temptable = getRdata('temptable');
beta = temptable(:,1);
betaStdErr = temptable(:,2);
tStats = temptable(:,3);


% evalR('R2 <- summarylm$r.squared',0);
% R2 = getRdata('R2');
% 
% evalR('R2A <- summarylm$adj.r.squared',0);
% R2A = getRdata('R2A');

evalR('y_hat <- fitted(model)',0);
y_hat = getRdata('y_hat');
y_hat = y_hat';

evalR('y_res <- residuals(model)',0);
y_res = getRdata('y_res');
y_res = y_res';



% plot regression results
evalR('plot(y, y_hat, xlab ="y", ylab="y_hat")',0);
% evalR('abline(lm(y_hat~y))',0);

% residual vs weight
evalR('windows()',0);
evalR('plot(y_res, weight, xlab ="Residual", ylab="weight")',0);

% scatter plot
evalR('windows()',0);
evalR('y = data.frame(y)',0);
evalR('all <- cbind(X,y)',0);
evalR('pairs(all)',0);




lm.y_hat = y_hat;
lm.y_res = y_res;
lm.beta = beta;
lm.betaStdErr = betaStdErr;
lm.tStats = tStats;
lm.weight = weight;

lm.df = df;
% lm.R2 = R2;
% lm.R2A = R2A;
lm.resStd = resStd;


end

